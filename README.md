...
Each agent's core lifecycle skill is authored in-house, but Product, Tech-lead, Builder, Verifier, and the security-reviewer specialist also have an optional, deeper vendored skill they can reach for on high-risk or otherwise warranted work (a requirements interview aid, legacy-codebase discovery, a broad PR-style delivery review, test generation and test-suite auditing, or SAST/pentest tooling) — see `SKILLS.md` for exactly which, sourced from where, and pinned to what version. Skill sourcing is confirmed closed to a 4-repo priority-ordered allowlist (also in `SKILLS.md`) — no other source is permitted, and two slots (an independent plan review for Tech-lead, and tagging/publishing an actual release) are honestly left unfilled because nothing in the allowlist is a real match yet.
...
- 4 roles, 1 default specialist, 5 in-house skills, 5 vendored supporting skills (pinned and reviewed, not a bulk import, sourced from a closed 4-repo priority allowlist), 5 doc templates, 3 CI templates. That's the whole kit — resist the urge to add a role, skill, or document for every idea; `CLAUDE.md` §3 and `INITIATION.md` §3 exist specifically to block that drift.
